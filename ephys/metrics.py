...
DEFAULT_WAVEFORMS = np.array([
    ...
    [
        ...
        22.4,
    ], # inverted waveform (prob interneuron)
])
...
def isi_violations_corrected(spike_train, min_time, max_time, isi_threshold, min_isi=0):
    ...
    sqrt_term = 1 - (num_violations * duration) / (num_spikes**2 * (isi_threshold - min_isi))
    if sqrt_term < 0:
        fpRate = 1
    else:
        fpRate = 1 - np.sqrt(sqrt_term)
    return fpRate
