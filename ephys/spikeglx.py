...
def plot_analog(
    filename, channel_idx=None, initial_sample_range=(5000, 10000), initial_gap=100
):
    def update_plot(start, window_size, gap):
        ax.clear()
        data = read_analog(
            filename, channel_idx=channel_idx, sample_range=(start, start + window_size)
        )
        # data -= np.median(data, axis=0, keepdims=True)
        # data -= np.median(data, axis=1, keepdims=True)
        ax.plot(
            np.arange(start, start + window_size)[:, np.newaxis],
        ...
        ax.set_yticklabels(
            channel_idx if channel_idx is not None else range(0, data.shape[1], 4)
        )
    ...
    sstart = Slider(
        ...
        "Time start",
        0,
        n_sample - initial_window_size,
        valinit=initial_sample_range[0],
        valstep=2500,
    )
    swindow = Slider(
        axwindow, "Time window", 2500, 60000, valinit=initial_window_size, valstep=2500
    )
...
def read_digital(filename, dtype="uint16"):
    ...
    n_channel = meta["nSavedChans"]
    sample_rate = (
        meta.get("imSampRate") or meta.get("niSampRate") or meta.get("obSampRate")
    )
    ...
    data_bits = np.unpackbits(data.view("uint8"), bitorder="little").reshape(
        -1, n_digital * np.dtype(dtype).itemsize * 8
    )
...
def read_bin(
    ...
    tprint(
        f"Loading {filename}, range={sample_range}, n_sample={n_sample}, offset={offset}"
    )
    data = np.memmap(
        filename, dtype=dtype, mode="r", shape=(n_sample, n_channel), offset=offset
    )
    if channel_idx is None:
        return data
    else:
    ...
    with open(filename_meta, "r") as f:
        for line in f:
            ...
            if key == "imroTbl":
                ...
                values = list(map(int, value.split(",")))
                meta[key] = {"AP": values[0], "LF": values[1], "SY": values[2]}
            elif key in ["acqXaDwSy", "snsXaDwSy"]:
                values = list(map(int, value.split(",")))
                meta[key] = {"XA": values[0], "DW": values[1], "SY": values[2]}
            else:
...
def parse_imrotbl(value):
    ...
    # Parse header: probe type (e.g. '24') or part number (e.g. 'NP2014')
    probe, num_channels = entries[0].split(",")
    num_channels = int(num_channels)
    try:
        probe_type, part_number = int(probe), None
        imro_format = PROBE_TYPE_TO_FORMAT.get(probe_type)
    except ValueError:
        probe_type, part_number = None, probe
        imro_format = PART_NUMBER_TO_FORMAT.get(part_number)
    if imro_format is None:
        raise ValueError(f"Unknown probe in imroTbl header: {probe!r}")

    channels = []
    for entry in entries[1:]:
        channel_data = tuple(map(int, entry.split()))

        if imro_format == "imro_np1000":  # NP 1.0-like
            channels.append(
                {
                    ...
                    "refid": channel_data[
                        2
                    ],  # Reference ID index (0=ext, 1=tip, [2..4]=on-shnk-ref)
            ...
            "imro_np2000",
            "imro_np2003",
        ):  # NP 2.0, single multiplexed shank
            channels.append(
                {
                    "channel": channel_data[0],  # Channel ID
                    "bank_mask": channel_data[
                        1
            ...
            # Note for imro_np2000: Reference ID values are {0=ext, 1=tip, [2..5]=on-shnk-ref}
            # On-shank ref electrodes are {127,507,887,1251}
            # Note for imro_np2003: Reference ID values are {0=ext, 1=gnd, 2=tip}
            # On-shank reference electrodes are removed from commercial 2B probes
        elif imro_format in ("imro_np2010", "imro_np2013"):  # NP 2.0, 4-shank
            channels.append(
                {
                    "channel": channel_data[0],  # Channel ID
                    "shank": channel_data[
                    ...
                    "electrode": channel_data[
                        4
                    ],  # Electrode ID (range [0,1279] on each shank)
                    "apgain": 80,
                }
            )
            # Note for imro_np2010: Reference ID values are {0=ext, [1..4]=tip[0..3], [5..8]=on-shnk-0, [9..12]=on-shnk-1, [13..16]=on-shnk-2, [17..20]=on-shnk-3}
            # On-shank ref electrodes of any shank are {127,511,895,1279}
            # Note for imro_np2013: Reference ID values are {0=ext, 1=gnd, [2..5]=tip[0..3]}
            # On-shank reference electrodes are removed from commercial 2B probes
        else:
            raise ValueError(f"Unsupported imro format: {imro_format}")

    return {
        "probe_type": probe_type,
        "part_number": part_number,
        "imro_format": imro_format,
...
def parse_snschanmap(value):
    ...
    return {"header": header, "channel_map": channel_map}
...
def parse_snsgeommap(value):
    ...
    for entry in matches[1:]:
        ...
        geom_data.append({"shank": s, "x": x, "z": z, "used": bool(u)})
...
def get_gain(meta):
    ...
    if meta["typeThis"] == "imec":
        if "imroTbl" not in meta or "channels" not in meta["imroTbl"]:
            raise ValueError(
                "Missing 'imroTbl' or 'channels' in metadata for imec recording"
            )

        if meta["fileName"].endswith(".ap.bin"):
            return np.array(
                [channel["apgain"] for channel in meta["imroTbl"]["channels"]]
            )
        elif meta["fileName"].endswith(".lf.bin"):
            return np.array(
                [channel["lfgain"] for channel in meta["imroTbl"]["channels"]]
            )
        else:
            raise ValueError(
                f"Unrecognized file type for imec recording: {meta['filename']}"
            )

    elif meta["typeThis"] == "nidq":
        if (
            "snsMnMaXaDw" not in meta
            or "niMNGain" not in meta
            or "niMAGain" not in meta
        ):
...
def get_uV_per_bit(meta):
    AiRangeMax = (
        meta.get("imAiRangeMax") or meta.get("niAiRangeMax") or meta.get("obAiRangeMax")
    )
...
def plot_chanmap(filename, save_file=True):
    ...
    if save_file:
        ...
        probe_fn = os.path.join(
            os.path.dirname(filename), f"{geom_data['header']['part_number']}.mat"
        )
    ...
    for i, (xi, zi) in enumerate(zip(x, z)):
        ax.annotate(
            str(i), (xi, zi), xytext=(3, -2), textcoords="offset points", fontsize=8
        )
...
def get_probe(meta: dict) -> dict:
    ...
    x = (
        electrodes["x"].values.astype(np.float32)
        + electrodes["shank"].values.astype(np.float32) * shank_spacing
    )
    y = electrodes["z"].values.astype(np.float32)
    connected = electrodes["used"].values.astype(np.bool_)

    channel_map = np.array(
        [i["channel"] for i in meta["snsChanMap"]["channel_map"]], dtype=np.int32
    )
